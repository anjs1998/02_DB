...
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE; -- 수업시간에 계정명이 언급된다면 이 구문을 필수적으로 실행할것
--VIEW 생성 권한 부여
...
-- D0 NULL L2
--VIEW에 INSERT를 수행했지만
--VIEW 생성시 사용한 원본 테이블에 값이 함께 INSERT됨을 확인

--모든 컬럼 값이 INSERT된것이 아니라
--VIEW를 생성할떄 사용된 컬럼에만 데이터가 삽입되었고
 ...
 * 
 * VIEW를 이용해서 DML을 사용하면 안된다!!!
 * */

/*
 * WITH READ ONLY 옵션 사용하기*/

--> VIEW를 이용해서 DML(INSERT/UPDATE/DELETE)를 막기 위해서!!
...
INCREMENT BY 5 -- NEXTVAL시 5씩 증가, 마이너스도 가능
MAXVALUE 150 -- 증가 가능한 최대값 150
NOMINVALUE --최소값 미지정
NOCYCLE --반복 안함
NOCACHE; -- 미리 만들어둘 시퀀스번호 X
...
-- CURRVAL의 정확한 의미는 가장 최근 호출된 NEXTVAL의 값을 반환함을 뜻함 NEXTVAL을 호출한적이 없어서 오류 발생
-- 생성되자마자 CURRVAL을 이용하면 안되고, NEXTVAL 호출후 CURRVAL을 이용해야 한다.

--> 해결방법 : NEXTVAL 호출하기
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
--시퀀스 생성 후 첫 NEXTVAL == START WITH값인 100
...
--SEQ_TEST_NO의 MAXVALUE값을 200으로 수정
...
FROM EMPLOYEE;
--ROWID : 오라클에거 각 행의 고유한 주소를 나타내는 가상 컬럼(물리적 주소)
-- 인덱스가 ROWID 저장함
...
CREATE TABLE TB_IDX_TEST(
	TEST_NO NUMBER PRIMARY KEY, -- 자동으로 UNIQUE INDEX 생성
	TEST_ID VARCHAR2(20) NOT NULL
	
);
--TB_IDX_TEST 테이블에 샘플데이터 100만개 삽입(PL/SQL사용)
-- * PL/SQL이란 오라클 데이터베이스에서 사용하는 절차적 확장 언어
-- 변수, 조건문(IF/CASE), 반복문(LOOP/WHILE) 사용 가능


BEGIN
	FOR I IN 1..1000000 -- I는 1부터 1000000까지 반복
	LOOP
...
SELECT * FROM TB_IDX_TEST
WHERE TEST_ID = 'TEST500000'; -- 0.020s
