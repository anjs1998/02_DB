...
-- 그룹함수는 단 한개의결과값만 산출하기 때문에 그룹이 여러개일 경우 오류 발생
-- 여러개의 결과값을 산출하기 위새 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술하여 사용
...
--SUBSTR에서 나온 값이 1이면 -> 남, 2이면 -> 여
--GROUP BY 해석순서가 SELECT보다 더 빠르기 때문에 GROUP BY에 SELECT에 쓴 별칭을 쓰면 안된다.
ORDER BY "인원 수";
...
SELECT DEPT_CODE, AVG(SALARY), COUNT(*)
FROM EMPLOYEE
--WHERE DEPT_CODE = 'D5' OR DEPT_CODE = 'D6'
WHERE DEPT_CODE IN ('D5', 'D6')
GROUP BY DEPT_CODE -- WHERE절에서 구한 부서코드가 D5, D6인 데이들을 그룹으로 묶음
ORDER BY DEPT_CODE;
...
FROM EMPLOYEE
WHERE HIRE_DATE > TO_DATE('1999-12-31')
--WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000
--WHERE SUBSTR( TO_CHAR(HIRE_DATE, 'YYYY'),1, 4) >= '2000'
GROUP BY JOB_CODE  -- WHERE절에서 구한 부서코드가 D5, D6인 데이들을 그룹으로 묶음
ORDER BY JOB_CODE;
...
-- ** GROUP BY 사용시 주의사항 **
--> SELECT문에 GROUP BY 절을 사용하는 경우
--SELECT절에 적은 컬럼 중
--그룹함수가 적용되지 않은 컬럼을
--모두 GROUP BY절에 빠짐없이 써놓아야 함
--안지키면 컴파일 오류가 나니, 규칙으로써 꼭 지킬것

--EMPLOYEE 테이블에서
--부서별로 같은 직급인 사원의 인원수를 조회(GROUP BY : DEPT_CODE, JOB_CODE)
...
FROM EMPLOYEE
GROUP BY DEPT_CODE , JOB_CODE  -- DEPT_CODE로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE로 그룹을 분류
...
-- HAVING 컬럼명 | 함수식 비교현산자 비교값

--EMPLOYEE 테이블에서
--부서별 편균 급여가 3백만원 이상인 부서의
--부서코드, 평균급여 조회
--부서코드 오름차순
...
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT(*) <= 5 -->주의!!: HAVING절에는 그룹함수가 반드시 작성된다!!
ORDER BY JOB_CODE;
...
--ROLLUP : GROUP BY 절에서 가장먼저 작성된 컬럼의 중간 깁계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE 
 ...
 * 
 * - MINUS(차집합) : A에서 A, B 교집1합 부분을 제거하고 조회
 * 
 * 
 * */
...
SELECT EMP_NO, EMP_NAME, SALARY FROM EMPLOYEE WHERE DEPT_CODE = 'D5'
UNION
SELECT EMP_NO, EMP_NAME, SALARY FROM EMPLOYEE WHERE SALARY > 3000000;
...
WHERE DEPT_CODE = 'D5'
UNION 
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '가나다' FROM EMPLOYEE WHERE SALARY > 3000000;
