...
-- * INSERT/UPDATE 시 컬럼값으로 DEFAULT 작성시 
...
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '김영희', '00123-2233445', SYSDATE);
--DEFAULT 설정된 컬럼이라고 삽입/수정 시 DEFAULT만 작성해야 하는것 아님!
...
CREATE TABLE USER_USED_NN(
	USER_NO NUMBER NOT NULL,--사용자번호(모든 사용자는 사용자번호가 있어야한다.)
													--컬럼레벨 제약조건 설정
	USER_ID VARCHAR2(20),
...
-- NULL은 UNIQUE 제약조건에도 불구하고 중복삽입 가능
...
CREATE TABLE USER_USED_PK(
	...
	-- 테이블 레벨
	-- CONSTRAINT USER_NO_PK UNIQUE(USER_NO)
);
...
--USER_NO, USER_ID 둘 다 중복되엇을 떄만 제약조건 위배 에러 발생


INSERT INTO USER_USED_PK2 VALUES(
2, 'USER01', 'PASS01', '고길동', '남자',
'010-1234-5678', 'hong_gd@or.kr'
);

INSERT INTO USER_USED_PK2 VALUES(
2, 'USER02', 'PASS01', '고길동', '남자',
'010-1234-5678', 'hong_gd@or.kr'
);


INSERT INTO USER_USED_PK2 VALUES(
NULL, 'USER02', 'PASS01', '고길동', '남자',
'010-1234-5678', 'hong_gd@or.kr'
); --NULL을 ("KH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다


INSERT INTO USER_USED_PK2 VALUES(
3, NULL, 'PASS01', '고길동', '남자',
'010-1234-5678', 'hong_gd@or.kr'
); --NULL을 ("KH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
--USER_GRADE(부모/참조테이블)에 10이라는 GRADE_CODE가 존재하므로 가능
...
CREATE TABLE USER_USED_FK2(
	...
	REFERENCES USER_GRADE2 ON DELETE SET NULL /*(GRADE_CODE)*/ -- 컬럼레벨
												--컬럼명 미작성시 USER_GRADE 테이블의 PK를 자동참조함.
...
--무결성 제약조건(KH.GRADE_CODE_FK)이 위반되엇습니다.


DELETE FROM USER_GRADE2 WHERE GRADE_CODE = 20;
--20 삭제됨

--> 부모 테이블인 USER_GRADE2에서 GRADE_CODE가 10 삭제
--> ON DELETE SET NULL 옵션이 ㅓㄹ정되어있어서
...
CREATE TABLE USER_USED_FK3(
	...
	REFERENCES USER_GRADE3 ON DELETE CASCADE /*(GRADE_CODE)*/ -- 컬럼레벨
												--컬럼명 미작성시 USER_GRADE 테이블의 PK를 자동참조함.
...
-- CHECK (컬럼명 비교연산자 비교값)
-- EX) GENDER -> CHECK( GENDER IN('남', '여') )   --> '남', '여'만 들어올수 있다. 
--            -> CHECK( 컬럼명 IS NULL )           --> NULL만 들어올수 있다.

CREATE TABLE USER_GRADE_CHECK(
	GRADE_CODE NUMBER PRIMARY KEY, -- 등급 번호
	GRADE_NAME VARCHAR2(30) NOT NULL -- 등급명
	
);

INSERT INTO USER_GRADE_CHECK VALUES(10, '일반회원');
INSERT INTO USER_GRADE_CHECK VALUES(20, '우수회원');
INSERT INTO USER_GRADE_CHECK VALUES(30, '특별회원');

SELECT * FROM USER_GRADE_CHECK;
...
INSERT INTO USER_USED_CHECK VALUES(
2, 'USER02', 'PASS02', '이순신', '남', 
'010-5678-1234'
,'lee_gd@or.kr');

INSERT INTO USER_USED_CHECK VALUES(
3, 'USER03', 'PASS03', '유관순', '여', 
'010-5678-1234'
,'lee_gd@or.kr');

INSERT INTO USER_USED_CHECK VALUES(
4, 'USER04', 'PASS04', '안중근', '남', 
'010-5678-1234'
,'lee_gd@or.kr');
...
DELETE FROM USER_GRADE_CHECK WHERE GRADE_CODE = 10;
