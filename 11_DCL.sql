 ...
 * 		UPDATE			 TABLE, VIEW
 * 		DELETE			 TABLE, VIEW
 * 		ALTER			   TABLE, SEQUENCE(수정권한 - 구조 변경)
 * 		REFERENCES 	 TABLE ( 참조 권한 - 외래키(FX) 설정할 권한)
...
-- 2. 새 연결(접속) 추가
CREATE USER ajs_sample IDENTIFIED BY 1234;

-- 3. 접속 권한 부여 (sys -> 사용자 계정)
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한, 권한, 권한.... TO 사용자명

GRANT CREATE SESSION TO ajs_sample;
...
-- CREATE TABLE : 테이블 생성 권한
-- + 데이터를 저장할수 있는 공간(TABLESPACE) 할당

-- 6. (sys) 테이블 생성 권한 부여 + TABLESPACE 할당

GRANT CREATE TABLE TO ajs_sample;
ALTER USER ajs_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
-- SYSTEM QUOTA UNLIMITED 
--> TABLESPACE 할당 크기 무제한
--> SYSTEM QUOTA 100M
--> TABLESPACE 할당 크기 100M
-- 7.
CREATE TABLE TB_TEST(
	PK_COL NUMBER PRIMARY KEY,
	CONTENT VARCHAR2(100)
...
-- CONNECT / RESOURCE : 사용자에게 부여할수 있는 기본적인 시스템 역할
...
-- (sys) sample 계정에 CONNEXT, RESOURCE ROLE 부여

GRANT CONNECT, RESOURCE TO ajs_sample;
------------------------------------------------------------
...
GRANT SELECT ON EMPLOYEE TO ajs_sample;

SELECT * FROM kh.EMPLOYEE; --> 조회 잘됨
SELECT * FROM kh.DEPARTMENT; --> 조회 안됨
--> 위에서 kh가 EMPLOYEE 조회하는 권한만 주엇기 떄문에..
...
REVOKE SELECT ON EMPLOYEE FROM ajs_sample;

SELECT * FROM kh.EMPLOYEE; --> 조회 안됨
