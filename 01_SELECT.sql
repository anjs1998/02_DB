...
--EMPLOYEE 테이블에서
-- 이메일 '_'앞이 4글자이면서
-- 부서코드가 'D9' 또는 'D6'이고 (참고 : AND가 OR보다 연산자 우선순위 높음, 소괄호 사용가능)
 ...
 * DB에서의 연산자 우선순위
 * 1. 산술 연산자 ( + - *  / )
 * 2. 연결 연산자 ( || ) -> 문자 앞뒤를 연결하는 연산자, 절대 OR가 아니다!!
 * 3. 비교 연산자 ( > < >= <= = != <>)
 ...
 * IN 연산자
 * 
 * 비교값과 목록에 작성된 값중 일치하는 것이 있으면 조회하는 연산자
 * 
 * 
 * [작성법]
...
-- EMPLOYEE 총 23명중 DEPT_CODE = NULL 미포함 2명
SELECT EMP_ID, EMP_NAME, DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D1', 'D6', 'D9', NULL)
--OR DEPT_CODE IS NULL; -- NULL 포함 총 14명 조회
 ...
 * ORDER BY 절
 * 
 * - SELECT 문의 조회 결과(RESULT SET)를 정렬할 떄 사용하는 구문
...
-- EMPLOYEEE 테이블(FROM 절)
-- 급여 오름 차순으로(ORDER BY절)
-- 사번, 이름, 급여 조회(SELECT 절)

SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY ; -- 기본값 : ASC(오름차순)가 기본값[ASC | DESC]
--컬럼명 사용

--EMPLOYEE 테이블에서 (FROM 절)
-- 급여가 200만 이상인 사원에서 (WHERE 절)
-- 사번, 이름, 급여 조회 (SELECT 절)
-- 단, 급여 내림차순으로 조회 (ORDER BY절)
