...
--> NOT NULL 제약조건은 새로운 CONSTRAINT 조건을 추가하는 것이 아닌
...
--다시 컬럼에 NULL 허용
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NULL; -- NOT NULL 해제
...
--DEPT_ID의 데이터 타입이 CHAR(2) 이므로 영어 1글자, 한글 1글자밖에 못들어가서 오류가 남
-- (D10은 3바이트, CHAR(2)에 들어갈수 없음)
...
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_TITLE; --SQL Error [12983] [72000]: ORA-12983: 테이블에 모든 열들을 삭제할 수 없습니다
--컬럼 삭제시 유의사항
-- 데이블 최소 1개 이상의 컬럼이 존재해야 하기때문에
-- 모든 컬럼을 다 삭제할순 없다.
...
CREATE TABLE TB2(
	...
	TB2_COL NUMBER REFERENCES TB1 -- FOREIGN KEY 제약조건 설정
...
DROP TABLE TB1; --SQL Error [2449] [72000]: ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--에러 해결방법
-- 1) 자식, 부모테이블 순서로 삭제
...
--> DDL이 종료된 후 DML 구문을ㅇ 수행할수 있도록 권장
