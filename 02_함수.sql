...
-- 함수는 SELECT문의
-- SELECT절, WHERE절, ORDER BY 절, GROUP BY절, HAVING절 사용가능
...
--문자열을 5번째 문자부터 검색하여 두번쨰 B위치까지 조회

SELECT INSTR('AABAACAABBAA', 'B', 5, 2) FROM DUAL; --10

--EMPLOYEE 테이블에서 사월명, 이메일, 이메일 중 @ 위치 조회
SELECT EMP_NAME, EMAIL, INSTR(EMAIL, '@')
FROM EMPLOYEE;

-- SUBSTR('문자열' | 컬럼명, 잘라내기 시작할 위치[, 잘라낼 길이])
-- 컬럼이나 문자열에서 지정한 우치부터 지정된 길이만큼 문자열을 잘라내서 반환
-- 잘라낼 길이 생략시 끝까지 잘라냄
...
SELECT TRIM(BOTH '#' FROM'######H E L L O######')
FROM DUAL;
SELECT TRIM(LEADING '#' FROM'######H E L L O######')
FROM DUAL;
SELECT TRIM(TRAILING '#' FROM'######H E L L O######')
FROM DUAL;
...
-- EMPLOYEE 테이블에서 사원의 원급을 100만으로 나눳을떄 나머지조회
...
SELECT ROUNLD(123.456) FROM DUAL;

--> 123.5 소수점 두번째 자리에서 반올림( 소수점 첫번째 자리까지표기)
...
--SYSTIMESTAMP : SYSDATE + ms 단위추가
SELECT SYSTIMESTAMP FROM DUAL;

--MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월수 차이 반환
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '2025-07-22'), 3) 
FROM DUAL; -- 4.371
...
EXTRACT(YEAR FROM HIRE_DATE) || '월' ||
EXTRACT(YEAR FROM HIRE_DATE) || '일'  AS 입사일
FROM EMPLOYEE
-----------------------------------------------------
--형변환 함수
...
SELECT TO_CHAR(SYSDATE, 'YYYY년 MM월 DD일 ') FROM DUAL -- 날짜 형식이 부적합합니다
--> 쌍따옴포를 이용하여 단순한 문자로 인식시키면 해결
...
SELECT TO_DATE('490505', 'RRMMDD') FROM DUAL; -- 1949-05-05 00:00:00.000
...
--2) 추출한 생년월일을 TO_DATE 타입으로 변경 -> R 패턴을 이용하여 1900년도로 변경(50년생 이하 없음)
...
--> ERROR : 콤마가 들어가서 문자열로 인식함
SELECT TO_NUMBER('1,000,000', '9,999,999') + 500000 FROM DUAL;


--NULL 처리 함수
--NVL(컬럼명, 컬럼값이 NULL 일떄 바꿀 값) : NULL인 컬럼값을 다른값으로 변경

SELECT * FROM EMPLOYEE;

--NULL과 산술 연산을 진행하면 결과는 무조건 NULL -> NVL()로 처리
...
-- EMPLOYEE 테이블에서 보너스를 받으면 'O', 안받느면 'X' 조회
SELECT EMP_NAME, NVL2(BONUS, 'O', 'X') "보너스 수령"
FROM EMPLOYEE;

-- 선택함수
-- 여러가지 경우에 따라 알맞은 결과를 선택할수 있음
-- DECODE(계산식 | 컬럼명, 조건값1, 선택값1, 조건값2, ... , 아무것도 일치하지 않을때)
...
-- 급여 최대값, 가장 최근 입사일, 알파벳순서가 가장 느림 이메일 조회
...
FROM EMPLOYEE 
WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE); --서브쿼리 활용
